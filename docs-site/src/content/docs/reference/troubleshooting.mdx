...
Cursor Origin needs the [Origin CLI](https://cursor.com/docs/origin/cli)
signed in (`origin auth status`). Native Windows is not supported. A missing
or old `origin` produces an Origin-specific error and never falls back to
`gh`.
