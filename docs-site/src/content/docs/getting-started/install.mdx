...
| Platform | Package |
| --- | --- |
| macOS (Apple Silicon) | `GitLane-X.Y.Z-macos-arm64-dmg.dmg` |
| macOS (Intel) | `GitLane-X.Y.Z-macos-x86_64-dmg.dmg` |
| Windows | `GitLane-X.Y.Z-windows-nsis.exe` |
...
<Tabs>
  ...
  <TabItem label="Linux">
    ...
    ```sh
    sudo apt install ./GitLane-X.Y.Z-linux-deb.deb      # Debian / Ubuntu / Mint
    sudo dnf install ./GitLane-X.Y.Z-linux-rpm.rpm      # Fedora / RHEL
    sudo zypper install ./GitLane-X.Y.Z-linux-rpm.rpm   # openSUSE
    ```
    ...
    ```sh
    chmod +x GitLane-X.Y.Z-linux-appimage.AppImage
    ./GitLane-X.Y.Z-linux-appimage.AppImage
    ```
