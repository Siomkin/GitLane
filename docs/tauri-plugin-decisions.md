...
| Opener | Installed | Keep. This is the preferred way to open external URLs/files from the frontend. It is deliberately **not** used for worktree paths: opening a repo file by its repo-relative path needs the backend's no-follow worktree guards (`git/worktree_fs.rs`) to resolve the leaf before any OS open, so those go through the typed `open_path_default` command instead (GL-337, `git/write/open_path.rs`). |
...
- `style-src 'self'; style-src-attr 'unsafe-inline'`: 127 `style={…}` React
  ...
  xterm.js injects a `<style>` element for cell font-size, ANSI color
  classes, cursor, and selection; that sheet is blocked here on purpose. The
  terminal pane sets `font-size` as a style *attribute* (`TERMINAL_FONT_SIZE`
  in `src/lib/ui.ts`) so glyphs inherit the app's 13px body size instead of
  the 16px browser default, and copies the xterm theme onto the pane as CSS
  custom properties (`applyXtermThemeVars`) that the bundled
  `src/features/terminal/xtermAnsi.css` maps onto `.xterm-fg-N` / `.xterm-bg-N`.
  Do not re-open `style-src` for that.
