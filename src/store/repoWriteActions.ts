...
> {
  ...
  const accountFor = (remote: string | null) =>
    remote ? useAccounts.getState().accountRefForRemote(remote) : null;
  ...
  return {
    ...
    pushBranch: (branch) =>
      runOp(get, async (summary) => {
        await api.pushBranch(summary.path, branch, accountFor(pushRemoteOf(branch)));
    ...
    publishBranch: (branch, upstream) =>
      runOp(get, async (summary) => {
        ...
        await api.publishBranch(summary.path, branch, upstream, accountFor(remote));
    ...
    deleteTag: (name, alsoRemote = false) =>
      runOp(get, async (summary) => {
        ...
        if (alsoRemote) {
          ...
          await api.deleteRemoteTag(summary.path, name, remote, accountFor(remote));
          try {
    ...
    pushTag: (name, remote) =>
      runOp(get, async (summary) => {
        ...
        await api.pushTag(summary.path, name, target, accountFor(target));
    ...
    deleteRemoteBranch: (remote, branch) =>
      runOp(get, async (summary) => {
        await api.deleteRemoteBranch(summary.path, remote, branch, accountFor(remote));
        return `Deleted ${remote}/${branch}`;
      }),

    forcePush: (branch) =>
      runOp(get, async (summary) => {
        await api.forcePush(summary.path, branch, accountFor(pushRemoteOf(branch)));
    ...
    fetch: async () => {
      ...
      try {
        ...
        const remoteAccounts = get()
          .remotes.map((r) => ({ remote: r.name, account: accountFor(r.name) }))
          .filter((pair): pair is { remote: string; account: NonNullable<typeof pair.account> } =>
            pair.account !== null,
          );
    ...
    push: async () => {
      ...
      try {
        ...
        await api.push(summary.path, accountFor(pushRemoteForBranch(head)));
        await get().refresh();
      } catch (e) {
        useUi.getState().showToast(String(e), "error");
      }
