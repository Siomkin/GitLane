...
describe("combinedRefPillModel", () => {
  ...
  it("styles current with the accent and everything else as a plain card", () => {
    const current = combinedRefPillModel("main", 1, true, null).className;
    const other = combinedRefPillModel("feature", 1, false, null).className;
    expect(current).toContain("bg-[var(--accent)]");
    expect(current).not.toContain("bg-white");
    expect(other).toContain("bg-white");
    expect(other).not.toContain("bg-[var(--accent)]");
  });
});
