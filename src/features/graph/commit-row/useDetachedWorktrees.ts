 ...
 * this it is invisible in the graph. The selector returns a JSON primitive so a
 * row only re-renders when its own worktree set actually changes. */
export function useDetachedWorktreesAt(commitId: string): DetachedWorktreeAt[] {
  const json = useRepo((s) => {
    const entries = s.worktrees
      .filter((wt) => !wt.branch && !wt.bare && !wt.prunable && wt.head === commitId)
