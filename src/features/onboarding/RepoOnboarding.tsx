...
//    open (`mode: overlay`) so clone / init / open are reachable mid-session.
...
type RepoOnboardingProps =
  | { mode: typeof ONBOARDING_MODE.Inline }
  | { mode: typeof ONBOARDING_MODE.Overlay; onClose: () => void };

export const RepoOnboarding = (props: RepoOnboardingProps) => {
  const onClose = props.mode === ONBOARDING_MODE.Overlay ? props.onClose : undefined;
  ...
  if (props.mode === ONBOARDING_MODE.Inline) {
