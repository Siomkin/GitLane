...
export function Toast() {
  ...
  return (
    <div
      ...
      aria-atomic="true"
      className="fixed bottom-[26px] left-1/2 z-[70] flex -translate-x-1/2 items-center gap-2.5 rounded-xl border border-black/10 bg-white px-[18px] py-[11px] shadow-[0_18px_44px_-8px_rgba(0,0,0,0.4)] dark:border-white/10 dark:bg-neutral-800"
      style={{ animation: "gp-toast .18s ease-out" }}
    >
      {toast.tone === "error" ? (
        <span className="grid h-4 w-4 place-items-center rounded-full bg-rose-500 text-[10px] font-bold text-white">
          !
        </span>
      ) : (
        <span className="grid h-4 w-4 place-items-center rounded-full bg-[var(--accent)] text-white">
          <CheckIcon className="h-2.5 w-2.5" />
        </span>
      )}
      <span className="text-[13px] text-neutral-800 dark:text-neutral-100">{toast.message}</span>
    </div>
  );
}
