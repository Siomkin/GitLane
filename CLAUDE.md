...
`pinning.ts` pure pin ordering, `useNavigatorSections` view-model hook, `useRowActions`
row-behaviour hooks, `rows/` presentational rows, `CategorySidebar`/`NavEmptyState`, and
the `BranchNavigator.tsx` container), shown in a 560px two-pane dropdown anchored under
...
pinned rows above; pins persist per repo in `ui.ts`'s `pinnedNavRefsByRepo`. The PR list
