...
See `proposal.md` for motivation and `specs/forge/origin/spec.md` for behavior. The current provider already routes every Origin operation through `forge::context()` and the single `run_origin` subprocess boundary. Its provider-neutral contract, IPC commands, pulls store, and Checks tab already cover the requested operations, but `OriginProvider` returns an empty check list, rejects approvals, and emits no submitted reviews in detail.

The current Cursor documentation and installed Origin CLI `2026.08.15-22-58-04-922a05a` expose `pr checks --json` and `pr review --approve`.
...
### Use direct CLI writes for bodyless approvals

Implement `OriginProvider::approve_pr` using `origin pr review <number> --approve -R <repo>`. The command accepts neither an action nor a body, so request-changes, comment-only reviews, and authored review text cannot cross the provider boundary.
...
### Expose bodyless approval through the shared PR action

Render the existing bodyless approval control for Origin so it reuses current pending state, confirmations, toasts, and refreshes. Comment and request-changes controls remain absent for every provider.
...
- [A newer CLI later supports request changes] → Keep text-authoring actions outside GitLane; users continue those workflows on the provider site.

## Migration Plan

No data migration, feature flag, dependency, or permission change is required. Ship the Origin provider and approval gate changes together; rollback restores the current empty Checks tab and hidden approval without altering repository or credential state.
