...
use super::AUTO_ALLOW_TOOL_KINDS;
...
/// Pick allow vs reject for a permission request from the tool call's `kind`.
pub(super) fn permission_outcome(params: Option<&Value>) -> Value {
    ...
    let tool_kind = params
        ...
        .and_then(Value::as_str)
        .unwrap_or("other");
    let want_allow = AUTO_ALLOW_TOOL_KINDS.contains(&tool_kind);
    let pick = |prefix: &str| {
        options
            .iter()
            .find(|option| {
...
mod tests {
    ...
    fn auto_allows_read_tool_permissions() {
        let ask = r#"{"jsonrpc":"2.0","id":77,"method":"session/request_permission","params":{"sessionId":"sess_1","toolCall":{"toolCallId":"c1","kind":"execute"},"options":[{"optionId":"reject-once","name":"Reject","kind":"reject_once"},{"optionId":"allow-once","name":"Allow once","kind":"allow_once"}]}}"#;
        let mut sent = Vec::new();
