...
impl GhCheck {
    ...
    /// A check passes on a SUCCESS conclusion/state; NEUTRAL/SKIPPED count as
    /// non-blocking. Anything still pending or failing reads as not-ok.
    pub(super) fn ok(&self) -> bool {
        if let Some(c) = &self.conclusion {
            return matches!(c.as_str(), "SUCCESS" | "NEUTRAL" | "SKIPPED");
        }
...
mod tests {
    ...
    fn check_passes_on_success_neutral_or_skipped_conclusion() {
        assert!(gh_check(None, None, Some("SUCCESS"), None).ok());
        assert!(gh_check(None, None, Some("NEUTRAL"), None).ok());
        assert!(gh_check(None, None, Some("SKIPPED"), None).ok());
    }

    #[test]
    fn check_fails_on_failure_conclusion() {
        assert!(!gh_check(None, None, Some("FAILURE"), None).ok());
        assert!(!gh_check(None, None, Some("TIMED_OUT"), None).ok());
        // A failing conclusion wins even when the state field looks healthy.
        assert!(!gh_check(None, None, Some("FAILURE"), Some("SUCCESS")).ok());
    }

    #[test]
    fn check_falls_back_to_state_when_no_conclusion() {
        assert!(gh_check(None, None, None, Some("SUCCESS")).ok());
        assert!(!gh_check(None, None, None, Some("PENDING")).ok());
        // No conclusion and no state reads as not-ok.
        assert!(!gh_check(None, None, None, None).ok());
    }
